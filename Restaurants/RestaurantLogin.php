...
if (!isset($_SESSION['is_restaurant_login'])) {
 if (isset($_REQUEST['rEmail'])) {
  ...
  $sql       = "SELECT r_email, r_password FROM restaurantlogin_tb WHERE r_email='" . $rEmail . "' AND r_password='" . $rPassword . "' limit 1";
  $result    = $conn->query($sql);
  if ($result->num_rows == 1) {

   $_SESSION['is_restaurant_login'] = true;
   $_SESSION['rEmail']              = $rEmail;
   // Redirecting to RequesterProfile page on Correct Email and Pass
   echo "<script> location.href='RestaurantProfile.php'; </script>";
   exit;
  } else {
